...
get_count_percent(data$species)


xtab <- table(data$sex, data$species)
class(xtab)
xtab
...
get_count_percent(perc_data$species)
...
g3 <- ggplot(mutate(data, species = fct_infreq(species)))+
  geom_bar(aes(x = species, fill = sex), position = "dodge")
  
g3

g4 <- ggplot(mutate(data, species = fct_infreq(species)))+
  geom_bar(aes(x = species, fill = sex), position = "dodge")+
  geom_text(mapping = aes(y = g_data$n, label = g_data$n), position = position_dodge(width = 0.9),
            cex = 2.5, vjust = -1)
