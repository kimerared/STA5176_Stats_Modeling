...
```{r}
blood_fx = aov(blood_data$sbp ~ factor(blood_data$BMI3cat) + factor(blood_data$everSmoker))
summary(blood_fx)
```
### Main effects for Health Levels
```{r}
TukeyHSD(blood_fx, "factor(blood_data$BMI3cat)")
```
With a p-value < 0.001, there is a significant difference between poor and ideal health levels.

### Main effects for Smoking Status
```{r}
TukeyHSD(blood_fx, "factor(blood_data$everSmoker)")
```
With a p-value = 0.02, there is a significant difference between smokers and non-smokers.
...
People who smoke scored higher in blood pressure results regardless of their health level. On the other hand, non-smokers had better blood pressure than the poor health group. 
The ideal health level group showcased that non-smokers with great physical activity had better blood pressure.
...
### Post-hoc comparisons for main effects
We used the parametric Tukey's honest significance (HSD) post-hoc test.

### Main effects for Health Levels
```{r, echo=FALSE}
TukeyHSD(blood_fx, "factor(blood_data$BMI3cat)")
```
With a p-value < 0.001, there was a significant difference between poor and ideal health levels.

### Main effects for Smoking Status
```{r, echo=FALSE}
TukeyHSD(blood_fx, "factor(blood_data$everSmoker)")
```
With a p-value = 0.02, there was a significant difference between smokers and non-smokers.