...
## Analisis
A Kruskal-Wallis test was conducted to find a statistically significant difference in BMI between the three levels of health (poor, intermediate, ideal).
Dunn's post-hoc test was performed which is the appropriate procedure following a Kruskal-Wallis test.
All three level of health grouped by BMI are significant different from each other. Poor health is extremely dominant.
Normality checks and Levene’s test were conducted to verify ANOVA assumptions; however, the assumptions we not met.
