...
The procedure following a Kruskal-Wallis test was to performed a Dunn's posthoc test.
All three levels of health grouped by BMI were significantly different from each other. Poor health was the dominant group.
...
The Fitted vs. Residuals graph showed how the scatterplots had differences in size among the groups.
...
ggplot(lm_model) + 
  ...
  theme_bw()
```
The QQ (quantile-quantile) plot  did not display the normality of residuals. The scatterplots did not follow the 45-degree line.
```{r, echo=FALSE}
# QQ plot
ggplot(lm_model, aes(sample = lm_model$residuals))+
  ...
  ylab("Standarized Residuals")
```
Moreover, a boxplot displayed the distribution of BMI health for each group.
```{r, echo=FALSE}
ggbetweenstats(
  data = clean_data,
  x = BMI3cat,
...
### Posthoc Dunn's Test
All groups had significant variance among each other, with a p-value < 0.001 with a 0.05 level of significance.
```{r, echo=FALSE}
# nonparametric DunnTest
DunnTest(clean_data$BMI ~ clean_data$BMI3cat)
```
