...
summary(h3_interaction_blood <- aov(blood_data$sbp ~ blood_data$BMI3cat * factor(blood_data$everSmoker)))
```
### Hypothesis
...
## Conclusion
Reject $H_0$ at the $\alpha$=0.05 level. There is sufficient evidence to suggest that there is an interaction between Health Levels and Smoking.

In other words, the relationship between Health Levels depends on the Smoking pattern.
No further testing is required because there is an interaction between Health Levels and Smoking.
...
```{r}
LeveneTest(blood_data$sbp ~ blood_data$BMI3cat * blood_data$everSmoker)
```
## ANOVA Assumption
```{r}
plot(h3_interaction_blood)
boxplot(blood_data$sbp ~ blood_data$BMI3cat:blood_data$everSmoker)
```
