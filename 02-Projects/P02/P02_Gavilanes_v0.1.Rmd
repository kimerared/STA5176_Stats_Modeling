...
**a. Find the appropriate summary statistics to summarize the data.**
```{r}
# mean by BMI and Smoke
smoke_mean <- tapply(blood_data$sbp,
                     INDEX = list(blood_data$BMI3cat, blood_data$everSmoker), 
                     FUN = mean)
formattable(as.data.frame(smoke_mean))
...
blood_model = blood_data$sbp ~ blood_data$BMI3cat * factor(blood_data$everSmoker)
summary(h3_interaction_blood <- aov(blood_model))
```
### Hypothesis
- $H_0$: There is no interaction between Health levels and Smoking\
- $H_1$: There is an interaction between Health Levels and Smoking 
...
In other words, the relationship between Health Levels depends on the Smoking status.
No further testing is required because there is an interaction between Health Levels and Smoking status.
...
### 2. Normality
```{r}
plot(h3_interaction_blood)
```
### 3. Equal Variance
### Levene's Test for Equality of Variances
```{r}
LeveneTest(blood_model)
```
Since the p-value is lower than the significance level of 0.05, the ANOVA assumption is invalid.
...
```{r}
kruskal.test(blood_data$sbp ~ blood_data$BMI3cat)
# kruskalmc(blood_data$sbp ~ blood_data$BMI3cat)
# pairwise.wilcox.test(blood_data$sbp, blood_data$BMI3cat,
#                      p.adjust.method = "bonferroni",
#                      exact = FALSE)
#DunnTest(blood_data$sbp ~ blood_data$BMI3cat)
# Tukey
model_factor <- aov(blood_data$sbp ~ factor(blood_data$BMI3cat) + factor(blood_data$everSmoker))
TukeyHSD(model_factor, "factor(blood_data$BMI3cat)" )
```

## Main Effects for Smoking Health
```{r}
kruskal.test(blood_data$sbp ~ blood_data$everSmoker)
# kruskalmc(blood_data$sbp ~ blood_data$everSmoker)
# pairwise.wilcox.test(blood_data$sbp, blood_data$everSmoker,
#                      p.adjust.method = "bonferroni",
#                      exact = FALSE)
<<<<<<< HEAD
# DunnTest(blood_data$sbp ~ blood_data$everSmoker)
# Tukey
TukeyHSD(model_factor, "factor(blood_data$everSmoker)" )
=======
DunnTest(blood_data$sbp~ blood_data$everSmoker)
?DunnettTest
>>>>>>> 6761db5b866b46b38889f512982ab7510d890dfd
```
