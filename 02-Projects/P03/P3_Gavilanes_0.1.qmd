...
- However, there are more outliers that can represent issues for the model.
...
# Plug in for Children
quantile(data$children, na.rm = TRUE)

# create lines graph for Children of 0, 2, 4
# Age on the X-axis
gg_data <- data %>%
  mutate(c0 = c1[1] + c1[2]*data$age + c1[3], + c1[4]*0,
         c2 = c1[1] + c1[2]*data$age + c1[3], + c1[4]*2,
         c4 = c1[1] + c1[2]*data$age + c1[3], + c1[4]*4)

# ggplot graph
gg_data %>% ggplot(aes(x = age, y = charges)) +
  geom_point(color = "darkgray", size=3, alpha = 0.4) + 
  geom_point(data = outlier_color, color = "magenta", size = 3) +
  geom_line(aes(y = c0), color = "orange") +
  geom_line(aes(y = c2), color = "cyan") +
  geom_line(aes(y = c4), color = "magenta", size = 1.5) +
...
# bmi on the x-axis
gg_data <- data %>%
  mutate(c0 = c1[1] + c1[2] + c1[3]*data$bmi, + c1[4]*0,
         c2 = c1[1] + c1[2] + c1[3]*data$bmi, + c1[4]*2,
         c4 = c1[1] + c1[2] + c1[3]*data$bmi, + c1[4]*4)

# ggplot graph
gg_data %>% ggplot(aes(x = bmi, y = charges)) +
  geom_point(color = "darkgray", size=3, alpha = 0.4) + 
  geom_point(data = outlier_color, color = "magenta", size = 3) +
  geom_line(aes(y = c0), color = "orange") +
  geom_line(aes(y = c2), color = "cyan") +
