...
# get percentages
long_age_table %>% 
  mutate(x_percent = (long_age_table$Value) / sum(long_age_table$Value))
long_age_table
...
# 
# CrossTable(long_age_table$rowname, long_age_table$Column,
#            prop.chisq = FALSE,
#            prop.r = FALSE,
#            prop.t = FALSE,
#            chisq = TRUE)
...
```{r}
# get percentages
percent_age_table <- long_age_table %>% 
  mutate(x_percent = (long_age_table$Value) / sum(long_age_table$Value)) %>% 
  mutate(x_percent = round(x_percent * 100, 2))
...
gg_age_table+
  labs(title = "Promotions by Age",
       ...
       x = "Age Groups",
       y = "Count")+
  geom_text(label = "3.63%", x = 1, y = 5, size = 4, colour = "white")+
  geom_text(label = "16.53%", x = 1, y = 40, size = 4, colour = "white")+
  geom_text(label = "4.84%", x = 2, y = 5, size = 4, colour = "white")+
  geom_text(label = "15.32%", x = 2, y = 40, size = 4, colour = "white")+
  geom_text(label = "11.69%", x = 3, y = 5, size = 4, colour = "white")+
  geom_text(label = "15.73%", x = 3, y = 40, size = 4, colour = "white")+
  geom_text(label = "13.71%", x = 4, y = 5, size = 4, colour = "white")+
  geom_text(label = "18.55%", x = 4, y = 40, size = 4, colour = "white")
...
```{r}
# get percentages
percent_age_table_2 <- long_age_table_2 %>% 
  mutate(x_percent = (long_age_table_2$Value) / sum(long_age_table_2$Value)) %>% 
  mutate(x_percent = round(x_percent * 100, 2))
...
gg_age_table_2+
  ...
  xlab("Age Groups")+
  ylab("Count")+
  geom_text(label = "15.32%", x = 1, y = 25, size = 4, colour = "white")+
  geom_text(label = "32.26%", x = 1, y = 100, size = 4, colour = "white")+
  geom_text(label = "18.55%", x = 2, y = 25, size = 4, colour = "white")+
  geom_text(label = "33.87%", x = 2, y = 100, size = 4, colour = "white")
```
...
In the age discrimination potential lawsuit, statistical and graphical results demonstrate the relationship between promotion depending on age. 
The promotions in smaller groups like 1A are in favor of the following ages: 30-39, and 40-49.
However, in 1B the data demonstrates that the promotions are almost equal among both large age groups.
