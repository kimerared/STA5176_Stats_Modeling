...
bonus_data %>% 
  ...
  geom_density(data = bonus_data, aes(color = Department), alpha = 0.25)+
  geom_vline(data = groups_median, aes(xintercept=Median, color=Department), 
             linetype="dashed", size = 1.1)+
  annotate("text", x=9, y=0.1, label="Median", angle=90, size=6, color=custom_purple)+
  ...
  scale_x_continuous(labels = scales::percent_format(scale = 1), limits = c(5,13))
...
## Conclusion
The mortgage department needs improvement. Perhaps the credit department can share insights into its business processes to help the mortgage department. The mortgage department needs further investigation.
...
data_long %>% 
  ...
  geom_vline(data = summary_long_table, aes(xintercept=Median, color=Test), 
             linetype="dashed", size = 1.1)+
  annotate("text", x=70, y=0.01, label="Median", angle=90, size=6, color= custom_purple) + 
  xlim(35,112)+
  ylab("Reading Test")+
