...
The data has failed the Levene's Test with a $p-value = 0.04688$. There is a Significant difference between the variances.
...
```
The QQ plot displays normality of residuals. There are a few outliers towards the right tail.
...
Since there are significant outliers, and the data is not normally distributed, it is adequate to use Kruskal-Wallis test.
...
density_plot
```
The following chart displays outliers for all three groups with a significant difference outlined between Herbicide A and B.
...
Since there are significant outliers, and the data is not normally distributed, it is reasonable to use the nonparametric Kruskal-Wallis test.
