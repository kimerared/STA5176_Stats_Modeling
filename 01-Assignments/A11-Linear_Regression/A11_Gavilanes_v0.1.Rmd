...
However, the variance graph does not look correct. Thus, a Brown-Forsythe-Levene test will be performed.

### Brown-Forsythe-Levene test
```{r}
leveneTest(Time ~ factor(data$NoItems), data = data, center = median)
```

**Hypotheses**

- $H_0: \sigma^2_{time} = \sigma^2_{items}$
...
- Keep $H_0$ at the $\alpha=0.05$ level. There is sufficient evidence to suggest that all variances are equal. Thus, we have met the variance assumption.
...
- $\rho$ = 0.93; which indicates a very strong correlation.
...
- The assumptions have not been met. Thus, the nonparametric Spearman's correlation test should be reported.
```{r}
# QQ plot: Time
qq_time <- data %>% 
