...
$$\hat{y} = 14.59 + 1.07 x$$
### Interpretation
Before considering number of items, we expect the time to be at 14.59 minutes.
...
## Conclusion
Reject $H_0$ at $\alpha = 0.05$. There is sufficient evidence to suggest that the number of items is a significant predictor for shipping time.
...
```
The Q-Q plot has a better alignment. Even though, the histogram is negatively shifted, it does not represent a concern. Therefore, we can suggest that the assumptions have been met and the model is valid.
