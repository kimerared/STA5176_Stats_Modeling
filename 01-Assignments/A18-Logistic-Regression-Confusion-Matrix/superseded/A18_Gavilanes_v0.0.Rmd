...
```{r}
# models comparison
anova(base_mod, lecture_mod,
      test = "Chisq")
```

## The Lecture model
```{r}
lecture_mod <- glm(Survived ~ Age + Pclass + Fare, 
          data = data, 
          family = binomial,
          na.action = na.exclude)
summary(lecture_mod)
c_lecture <- coefficients(lecture_mod)
```
$$
\ln \left( \frac{\hat{\pi}}{1-\hat{\pi}} \right) = `r round(c_lecture[1],3)` `r round(c_lecture[2],3)` \text{ age} + `r round(c_lecture[3],3)` \text{ fare} `r round(c_lecture[4],3)`0 \text{ class}
$$
- Age (p < 0.001), and Passenger Class (p < 0.001) are significant predictors of survival.
- Fare (0.167) is not a significant predictor of survival. 
...
clean_data <- subset(data,select=c(2,3,5,6,7,8,10))
...
```
```{r}
# remove Fare; not significant predictor
m3 <- glm(clean_data$Survived ~ clean_data$Age + clean_data$Pclass)
