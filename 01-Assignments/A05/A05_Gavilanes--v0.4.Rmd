...
mortgage_qq <- bonus_data %>% 
  ...
  ggplot(aes(sample = Bonus)) +
    stat_qq(size=3, alpha = 0.6, color = "magenta") +
    stat_qq_line() +
    theme_minimal() +
...
## Conclusion
The Wilcoxon nonparametric test should be reported since the data is not normally distributed.
During the assumption, the QQ plot has shown some discrepancies. Especially for the credit department, there are some outliers towards the right tail.
...
## Conclusion
Keep H0 at the $\alpha$=0.05 level. There is not sufficient evidence to suggest that the program raises comprehension test averages by at least 5 points.
